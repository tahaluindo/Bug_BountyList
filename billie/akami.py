...
def domain_scanner(sub_domnames):
    ...
    for subdomain in sub_domnames:
        ...
        try:
            ...
            url=url, headers=default_headers, cookies=cookies, timeout=5, allow_redirects=True, verify=False)

            if sending_request.status_code == 400:
                print("\n")
                print(Fore.GREEN+url)
                print("\n")
                print("Supports Akami (/) Method")
                print("\n")
                print(sending_request.headers)
                space_line()
                response = requests.head(url)                                                                                                                               
                d = open("akami-result.txt", "a")                                                                                                                               
                d.name.splitlines()   
