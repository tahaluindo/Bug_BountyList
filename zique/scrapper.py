...
def domain_scanner(sub_domnames):                                                                                                                                     
    ...
    for subdomain in sub_domnames:                                                                                                                                    
        ...
        try:                                                                                                                                                          
            ...
            sending_request = requests.get(url = url, headers = default_headers, cookies = cookies , timeout=5,allow_redirects=True,verify=False)    

            soup = BeautifulSoup(sending_request.text, 'html.parser')
            ...
            for title in soup.find_all('title'):
                ...
                if "GitLab" in title:
                    space_line()
                    print("\n"+url+"\n")
                    api_endpoint = f'{url}/api/v4/projects'
                    endpoint_request = requests.get(url = api_endpoint, headers = default_headers, cookies = cookies , timeout=5,allow_redirects=True,verify=False)    
                    content = endpoint_request.content
                    print(content)
                    print("\n")
                    f = open("git-scrapt-result.txt", "a")                                                                                                                               
                    f.name.splitlines()                                                                                                                                       
                    f.write(url + " : "+ "\n" + content +"\n")                                                                                                                      
                    f.close()     
